...
for i, common_op_name in enumerate(common_op_names):
    if np.var(op_model_speedup[i]) > 5 :
        if m >= len(markers):
            m = 0
        plt.plot(model_names, op_model_speedup[i], label=common_op_name.split("::")[-1], color='k', linestyle="-", marker=markers[m], markersize=8)
...
plt.legend(loc='center left', bbox_to_anchor=(0.0, 1.0), ncol=3, frameon=False, fontsize=fontsize, columnspacing=0.4)
plt.xlabel("model", fontsize=fontsize)
plt.ylabel("speedup rate", fontsize=fontsize)
